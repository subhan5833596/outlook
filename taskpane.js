...
Office.onReady(function (info) {
    ...
    if (info.host === Office.HostType.Outlook) {
        ...
        console.log("✅ Supported account and item type — initializing UTM Manager.");
        populateFields();
        document.getElementById("updateUrlBtn").onclick = updateUTMInSignature;
    }
});
...
async function updateUTMInSignature() {
    ...
    Office.context.mailbox.item.body.getAsync("html", function (res) {
        ...
        signatureHtml = signatureHtml.replace(
            ...
            (match, url) => {
                try {
                    ...
                    if (meetingDomains.some((domain) => lowerUrl.includes(domain)) || lowerUrl.includes("meeting")) {
                    ...
                    ["utm_campaign", "utm_source", "utm_medium", "utm_content", "utm_term"].forEach((p) =>
                        params.delete(p)
                    );
        ...
        body = body.replace(sigMatch[0], `<div id="custom-signature">${signatureHtml}</div>`);
        ...
        Office.context.mailbox.item.body.setAsync(
            ...
            function (setRes) {
                if (setRes.status === Office.AsyncResultStatus.Succeeded) {
                    console.log("✅ UTM updated successfully — only signature links modified.");
                } else {
    ...
    if (!quill) {
        ...
        (async () => {
            try {
                ...
                if (savedSig) quill.root.innerHTML = savedSig;
            } catch (err) {
